...
export default function Footer({ lang = 'fr', agency }) {
  ...
  const footerLinks = {
    navigation: [
      { label: 'Accueil', href: '/' },
      { label: 'En Stock', href: '/stock' },
      { label: 'Sur Commande', href: '/commande' },
      { label: 'Location', href: '/location' },
      { label: 'Contact', href: '/contact' },
    ],
    services: [
      { label: 'Achat', href: '/stock' },
      { label: 'Import', href: '/commande' },
      { label: 'Location', href: '/location' },
      { label: 'Financement', href: '/contact' },
    ],
  };

  const phoneNumbers = agency?.phone ? [agency.phone] : ['0560 00 31 02', '0560 00 31 06', '0560 00 31 08'];
  const emailDisplay = agency?.email || 'contact@amineauto.dz';
  const addressDisplay = agency?.address || dict.footer.address;
  const companyName = agency?.company_name || 'Amine Auto';
      ...
      <div className={`container ${styles.grid}`}>
        ...
        <div className={styles.brand}>
          <div className={styles.brandLogo}>
            {agency?.logo_url ? (
              <Image src={agency.logo_url} alt={`${companyName} Logo`} width={160} height={60} style={{ objectFit: 'contain' }} />
            ) : (
              <div className={styles.brandText} style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', fontWeight: 700, color: '#fff' }}>
                {companyName}<span style={{ color: 'var(--color-primary)' }}>35</span>
              </div>
            )}
          </div>
        ...
        <div className={styles.column}>
          ...
          <ul className={styles.links}>
            {footerLinks.navigation.map((link) => (
              <li key={link.href}>
                <Link href={getLinkHref(link.href)}>{link.label}</Link>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul className={styles.contactList}>
            {phoneNumbers.map((phone, idx) => (
              <li key={idx} className={styles.contactItem}>
                ...
                <span dir="ltr" className="inline-block" style={{ unicodeBidi: 'plaintext' }}>{phone}</span>
              </li>
            ))}
      ...
      <div className={styles.bottom}>
        <div className="container">
          <p className={styles.copy}>
            © {new Date().getFullYear()} {companyName}. {dict.footer.copyright}
          </p>
        </div>
      </div>
    </footer>
