'use client';

import { useState, useEffect } from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { useRouter, usePathname } from 'next/navigation';
import styles from './Navbar.module.css';
import fr from '@/components/website/templates/auto-am/dictionaries/fr.json';
import ar from '@/components/website/templates/auto-am/dictionaries/ar.json';

import { useParams } from 'next/navigation';

export default function Navbar({ lang = 'fr', agency }) {
  const router = useRouter();
  const pathname = usePathname();
  const params = useParams();
  const subdomain = params?.subdomain;
  const basePath = subdomain ? `/${subdomain}` : '';

  const dict = lang === 'ar' ? ar : fr;
  const [scrolled, setScrolled] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 20);
    };
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  const navLinks = [
    { label: 'Accueil', href: '/' },
    { label: 'En Stock', href: '/stock' },
    { label: 'Sur Commande', href: '/commande' },
    { label: 'Location', href: '/location' },
    { label: 'Contact', href: '/contact' },
  ];

  const getLinkHref = (path) => {
    if (path === '/') return basePath || '/';
    return `${basePath}${path}`;
  };

  const isActive = (path) => {
    if (path === '/') {
      return pathname === basePath || pathname === `${basePath}/`;
    }
    return pathname?.startsWith(`${basePath}${path}`);
  };

  const phoneDisplay = agency?.phone || '0560 00 31 06';
  const whatsappNumber = phoneDisplay.replace(/\s+/g, '').replace(/^0/, '213');

  return (
    <header
      className={`${styles.navbar} ${scrolled ? styles.scrolled : ''}`}
      id="main-navbar"
    >
      <div className={`container ${styles.inner}`}>
        <Link href={getLinkHref('/')} className={styles.logo} id="nav-logo">
          {agency?.logo_url ? (
            <Image src={agency.logo_url} alt={`${agency.company_name} Logo`} width={140} height={50} style={{ objectFit: 'contain' }} priority />
          ) : (
            <div className={styles.logoText}>
              {agency?.company_name || 'Amine Auto'}<span className={styles.logoAccent}>35</span>
            </div>
          )}
        </Link>

        <nav
          className={`${styles.nav} ${menuOpen ? styles.navOpen : ''}`}
          id="nav-menu"
        >
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={getLinkHref(link.href)}
              className={`${styles.navLink} ${isActive(link.href) ? styles.activeLink : ''}`}
              onClick={() => setMenuOpen(false)}
            >
              {link.label}
            </Link>
          ))}
        </nav>

        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <button 
            onClick={() => {
              if (!pathname) return;
              const segments = pathname.split('/');
              segments[1] = lang === 'fr' ? 'ar' : 'fr';
              router.push(segments.join('/'));
            }}
            style={{
              background: 'var(--color-surface)',
              border: '1px solid var(--color-border)',
              color: 'var(--color-text)',
              padding: '6px 12px',
              borderRadius: '20px',
              fontSize: '0.85rem',
              fontWeight: '600',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: '6px',
              transition: 'all 0.3s ease'
            }}
            onMouseOver={(e) => { e.target.style.borderColor = 'var(--color-primary)'; e.target.style.color = 'var(--color-primary)'; }}
            onMouseOut={(e) => { e.target.style.borderColor = 'var(--color-border)'; e.target.style.color = 'var(--color-text)'; }}
          >
            {lang === 'fr' ? 'AR' : 'FR'}
          </button>
          <a
            href={`https://wa.me/${whatsappNumber}`}
            target="_blank"
            rel="noopener noreferrer"
            className={styles.ctaBtn}
          >
            <svg className={styles.ctaIcon} viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            <span style={{ fontSize: '0.9rem', letterSpacing: '0.4px' }}><span dir="ltr" className="inline-block" style={{ unicodeBidi: 'plaintext' }}>{phoneDisplay}</span></span>
          </a>
        </div>

        <button
          className={`${styles.hamburger} ${menuOpen ? styles.hamburgerOpen : ''}`}
          onClick={() => setMenuOpen(!menuOpen)}
          aria-label="Toggle menu"
          id="nav-hamburger"
        >
          <span />
          <span />
          <span />
        </button>
      </div>
    </header>
  );
}
