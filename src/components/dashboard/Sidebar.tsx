...
import { LogOut, ChevronLeft, ChevronRight, MessageSquare, Flame, Users, Car, Users2, Bot, Globe, Settings } from 'lucide-react'
...
export function Sidebar({ 
  ...
  // Hover-expand logic: Expanded if not collapsed OR if hovered
  ...
  } else if (pathname.includes('/dashboard/trips') || pathname.includes('/dashboard/visa') || pathname.includes('/dashboard/bookings') || pathname.includes('/dashboard/finance') || pathname.includes('/dashboard/invoices')) {
  ...
  return (
    <div 
      ...
      className={cn(
        "hidden lg:flex flex-col shrink-0 border-r border-slate-200/80 bg-white h-screen sticky top-0 justify-between select-none transition-all duration-300 ease-in-out z-20 shadow-xs",
    ...
    >
                      ...
                      <div className="flex items-center gap-1.5 ml-auto">
                        ...
                        {badgeVal > 0 && (
                          <Badge className={cn(
                            ...
                            {item.showGlobe && <Globe className="h-2.5 w-2.5 animate-spin-slow shrink-0" />}
                            {badgeVal}
                          </Badge>
                        )}
                      </div>
      ...
      <div className="p-4 border-t border-slate-100 flex items-center justify-between">
        ...
        <button 
          ...
          className={cn(
            "h-8 w-8 rounded-lg hover:bg-slate-50 text-slate-400 hover:text-slate-655 hover:text-slate-600 transition flex items-center justify-center active:scale-95 cursor-pointer",
            !isExpanded && "mx-auto"
          )}
