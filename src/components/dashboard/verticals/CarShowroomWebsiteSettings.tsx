...
export function CarShowroomWebsiteSettings({ salesCars = [], rentalCars = [] }: { salesCars?: any[], rentalCars?: any[] }) {
  ...
  // Initialize with fallback to car showroom template
  const [formData, setFormData] = useState<any>(() => {
    const defaultTemplate = templatesList.find(t => t.business_type_slug === 'car_showroom') || templatesList[0]
    const config = (websiteConfig || defaultTemplate) as any
    
    // Detect stale travel config for car_showroom verticals: if saved sections contain travel-only types, force reset
    const savedSections = config.structure?.sections || [];
    ...
    const needsCarReset = (hasTravelSections || hasNoCarSections) && savedSections.length > 0;
    
    return {
      active_template_id: needsCarReset ? defaultTemplate.id : (config.active_template_id || defaultTemplate.id),
      global_styles: needsCarReset ? { ...defaultTemplate.global_styles } : { ...defaultTemplate.global_styles, ...(config.global_styles || {}) },
      structure: { 
        sections: needsCarReset ? defaultTemplate.structure.sections : (config.structure?.sections || defaultTemplate.structure.sections)
      },
  ...
  const SECTION_LIBRARY = [
    { type: 'Navbar', name: 'Showroom Navigation', icon: Layers, desc: 'Top branding navbar with links and phone buttons', defaultVariant: 'center' },
    { type: 'Hero', name: 'Hero Banner', icon: Sparkles, desc: 'Premium showroom banner with title, subtitle, and search CTA', defaultVariant: 'center' },
    { type: 'Stats', name: 'Numeric Stats', icon: Clock, desc: 'Showroom metrics like vehicles delivered, client satisfaction, financing approval', defaultVariant: 'center' },
    { type: 'Car Grid', name: 'Car Grid', icon: Car, desc: 'Real-time inventory grid showing active cars on sale or order', defaultVariant: 'center' },
    { type: 'Services', name: 'Dealership Services', icon: Award, desc: 'Highlight purchase options, import on order, and premium rental', defaultVariant: 'center' },
    { type: 'Testimonials', name: 'Client Testimonials', icon: Star, desc: 'Customer feedback quotes and reviews with ratings', defaultVariant: 'center' },
    { type: 'Contact', name: 'Contact Form', icon: Mail, desc: 'Lead capture forms, coordinates, and contact details', defaultVariant: 'left' },
    { type: 'Map', name: 'Interactive Map', icon: MapPin, desc: 'Google map locator showing dealership physical location', defaultVariant: 'center' },
    { type: 'Text', name: 'Rich Editorial Paragraph', icon: MessageSquare, desc: 'Rich text block or custom about us paragraph', defaultVariant: 'centered' },
    { type: 'Footer', name: 'Global Footer', icon: Layers, desc: 'Branded navigation footer with coordinates and copyrights', defaultVariant: 'center' }
  ];
      ...
      case 'Testimonials':
        return {
          ...
          items: [
            ...
            { name: 'Amel Belkacem', quote: 'J\'ai loué une berline de luxe pour mon mariage. Le service était impeccable.', rating: 5 }
          ]
        };
      case 'Contact':
        return {
                  ...
                  <div key={sec.id} className="space-y-3 w-full">
                    ...
                    >
                      ...
                      <div className="pointer-events-auto">
                        <PublicSite 
                          agency={{ ...agencyInfo, business_type_slug: 'car_showroom' }} 
                          trips={[]} 
                          salesCars={salesCars}
          ...
          <Card className="bg-slate-900 border border-slate-800 rounded-3xl p-3 shadow-xl">
            ...
            <CardContent className="p-0 overflow-hidden bg-slate-850 flex items-center justify-center min-h-[460px] relative">
              ...
              >
                ...
                <PublicSite 
                  agency={{ ...agencyInfo, business_type_slug: 'car_showroom', website_config: formData }} 
                  trips={[]} 
                  salesCars={salesCars}
                  rentalCars={rentalCars}
                  isEditing={false} 
